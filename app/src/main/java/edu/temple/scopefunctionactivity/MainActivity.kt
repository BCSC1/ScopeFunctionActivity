...
class MainActivity : AppCompatActivity() {
    ...
    private fun averageLessThanMedian(listOfNumbers: List<Double>): Boolean =
        listOfNumbers.sorted().let { sortedList ->
            val median = when (sortedList.size % 2) {
                0 -> (sortedList[sortedList.size / 2] + sortedList[(sortedList.size - 1) / 2]) / 2
    ...
    private fun getView(position: Int, recycledView: View?, collection: List<Int>, context: Context): View =
        when (recycledView) {
            is TextView -> recycledView
            else -> TextView(context).apply { setPadding(5, 10, 10, 0); textSize = 22f }
